...
landis_fuels <- raster("./calibration data/landfire/fine-fuels-6.tif")

# LANDFIRE fine fuels for 2019
landfire_2019 <- raster("D:/Data/lf89339302_US_200F40_19/US_200F40_19/us_200f40_19/w001001.adf")
...
fuel_lookup <- read.csv("./calibration data/landfire/fuel_loads_scott_burgan.csv") %>%
  mutate(total_fuels = X10hr + X100hr + Live_woody) %>% # think about which fuel variables to use
...
values(fuel_2019) <- fuel_lookup$total_fuels[values(fuel_2019)]
values(fuel_2019) <- values(fuel_2019) * 224.17 #convert to g m-2
...
values(fuel_2019) <- ifelse(values(landis_fuels) == 0, NA, values(fuel_2019))
values(landis_fuels) <- ifelse(values(landis_fuels) == 0, NA, values(landis_fuels))

# plot(values(landis_fuels) ~ values(fuel_2019))
# mod <- lm(values(landis_fuels) ~ values(fuel_2019))
# summary(mod)
# abline(coef(mod))
mod <- lm(values(landis_fuels)~ values(fuel_2019)  + 0)
summary(mod)
...
landfire_2001 <- raster("D:/Data/lf06186785_US_105FBFM40/US_105FBFM40/us_105fbfm40/w001001.adf")
...
landfire_2014 <- raster("D:/Data/lf23669231_US_140FBFM40/US_140FBFM40/us_140fbfm40/w001001.adf")
...
landfire_2020 <- raster("D:/Data/lf34933400_US_200F40_20/US_200F40_20/us_200f40_20/w001001.adf")
...
landfire_2021 <- raster("D:/Data/lf65319260_US_210F40_21/US_210F40_21/us_210f40_21/w001001.adf")
...
# translate to fine fuels
landfire_mod_2019 <- raster::match(landfire_2019, fuel_lookup$ID)
values(landfire_mod_2019) <- fuel_lookup$total_fuels[values(landfire_mod_2019)]
values(landfire_mod_2019) <- values(landfire_mod_2019) * 224.17 #convert to g m-2
