...
shapefile_list_subset <- shapefile_list %>%
  purrr::map(., ~ dplyr::select(., c("incidentna", "perimeterd", "fireyear", "gisacres", "complexnam", "uniquefire", "comments"))) %>%
  purrr::map(., ~ dplyr::mutate(., perimeterd = as.character(perimeterd)))

perims_2000_2019 <- sf::st_as_sf(data.table::rbindlist(shapefile_list_subset, use.name = TRUE, fill = TRUE))
