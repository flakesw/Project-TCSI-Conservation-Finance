...
year_model <- glmer(success ~ scale(fwi) + scale(fuel) + scale(eff_wspd) + scale(year) + (1|fire_name),
                data = spread_data,
                family = "binomial")
year_model <- glmer(success ~ scale(fwi)*scale(year) + scale(fuel)*scale(year) + 
...
interaction_model <- glm(success ~ scale(fwi)*WIPGeoNam.1 + scale(fuel)*WIPGeoNam.1 + scale(eff_wspd)*WIPGeoNam.1,
                    data = sf::as_Spatial(sf::st_as_sf(spread_data_loc)),
                    family = "binomial")
...
preds <- predict(random_model, newdata = newdata[1:200, ], re.form=NA)
preds_re <- predict(random_model, newdata = newdata)
