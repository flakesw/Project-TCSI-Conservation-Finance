...
values(management_zones2)[values(management_zones2) == 0] <- NA
management_zones2 <- terra::focal(management_zones2, w=9, 
                                   fun= modal, 
                                   na.policy="only",
                                   na.rm = TRUE) %>%
  terra::crop(tcsi_mask_terra)
...
stands3_vect$slope <- terra::extract(slope_percent, 
                                        vect(stands3_vect), 
                                        fun = "mean")$slope

stands3_vect <- sf::st_cast(stands3_vect, "MULTIPOLYGON") %>% sf::st_cast("POLYGON") #convert to polygon
...
stands3_vect$MZ <- as.numeric(ifelse(stands3_vect$slope > 0.3, 
                             stands3_vect$MZ + 1, 
                             stands3_vect$MZ))
table(stands3_vect$MZ)
plot(stands3_vect["MZ"])

management_zone_raster <- terra::rasterize(stands3_vect, tcsi_mask_terra, "MZ") 
plot(management_zone_raster)
table(values(management_zone_raster))
unique(values(management_zone_raster))[order(unique(values(management_zone_raster)))]
...
#all combinations of management zones
mz <- readxl::read_xlsx("./Models/Inputs/management_scenarios/sdi_35_full_template.xlsx")
map_zones <- unique(values(management_zone_raster))[order(unique(values(management_zone_raster)))] 
map_zones[!(map_zones %in% mz$`Mgmt Area`)]

expand.grid(list(2:7, 0:2, 0:2, 0:1))
