...
scenarios <- data.frame(scenario_name = c("subset_scenario3 - new suppression test spread",
                                          "subset_scenario3 - new suppression test spread - Copy"),
               climate = c(rep("Historical", 2)),
               management = rep(c("Moderate"), 2),
...
historical <- read.csv("./Scenario1/Climate-future-input-log.csv") %>%
  group_by(Year) %>%
  filter(Timestep > 210 & Timestep < 300) %>%
  summarise(mean_fwi = mean(FWI),
            max_fwi = max(FWI))

miroc <- read.csv("./Scenario1 - miroc/Climate-future-input-log.csv")%>%
  group_by(Year) %>%
  filter(Timestep > 210 & Timestep < 300) %>%
  summarise(mean_fwi = mean(FWI),
            max_fwi = max(FWI))
