...
#raster overlap
rast_names <- paste0("fire-intensity-", 1:20, ".img")
...
fire_severity <- calc(fire_stack, max)

fire_reproject <- project_to_template(fire_severity, grid)

writeRaster(fire_reproject, "./test/scenario1_severity2.tif", overwrite = TRUE)
...
mtbs_mosaic <- stack("./calibration data/mtbs/severity_mosaic/combined_mosaic.tif")

mtbs_mosaic_test <- raster::calc(mtbs_mosaic, max)

plot(mtbs_mosaic_test)

writeRaster(mtbs_mosaic_test, "./calibration data/mtbs/severity_mosaic/flattened_mosaic_max.tif")
