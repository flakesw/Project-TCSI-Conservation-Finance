...
abg_proj[is.na(tcsi_mask_9311)] <- NA
plot(abg_proj)

totalc_init <- abg_proj*1.5 + carbon_init_proj
plot(totalc_init)
totalc_init[is.na(tcsi_mask_9311)] <- NA
...
writeRaster(abg_out, "./Analysis/example biomass layers/aboveground_biomass_initial.tif")
writeRaster(somtc_out, "./Analysis/example biomass layers/soil_organic_carbon_initial.tif")
writeRaster(totalc_out, "./Analysis/example biomass layers/total_carbon_initial.tif")
