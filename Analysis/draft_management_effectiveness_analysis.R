...
select_severe_fire <- function(raster){
  mat <- matrix(c(0, 300, 0, 300, 2001, 1), ncol = 3, byrow = TRUE)
...
#-------------------------------------------------------------------------------
mgmt_dirs <- paste0("E:/TCSI LANDIS/LANDIS Runs/Scenario1 - miroc - Run ", c(1:5),"/harvest/")
...
# biomass removed
mgmt_biomass_sums1 <- mgmt_biomass_runs_list %>%
  purrr::modify(.f = ~ terra::app(.x, fun=sum))

#convert the list to a raster with layers, and take the average across all the 
# model runs
mgmt_total_biomass1 <- collapse_list_to_layers(mgmt_biomass_sums1) %>%
  terra::app(fun=sum)%>%
  `/`(length(mgmt_biomass_sums1))
plot(mgmt_total_biomass1)
...
fire_dirs <- paste0("E:/TCSI LANDIS/LANDIS runs/Scenario1 - miroc - Run ", c(1:5),"/social-climate-fire/")
...
severe_frequency <- wildfire_biomass_stack %>%
  purrr::lmap(.f = ~ lapply(.x, FUN = select_severe_fire)) %>% #TODO make sure we're doing what we want to here
  purrr::modify(.f = ~ terra::app(.x, fun=function(x){sum(x, na.rm = TRUE)}))
...
biomass_with_rx_sums1 <- mgmt_biomass_sums1 %>%
...
plot(mgmt_biomass_sums1[[1]])
plot(biomass_with_rx_sums1[[1]]/100)
...
biomass_diff <- mgmt_biomass_sums6 %>%
  purrr::modify(.f = ~ .x - mgmt_total_biomass1)
...
scenario1_biomass_treated <- project_to_template(biomass_with_rx_sums1[[1]]/100, tcsi_mask_terra)
